...
test.describe("initial test", () => {
  ...
  test("Mock test", async ({ page }) => {
    ...
    await page.route(`http://localhost:5000/addToDo`, async (route) => {
      const json = {
        todos: [
          {
            id: 1735281073039,
    ...
    await expect(page.getByRole("list")).toContainText("Task 1");
  });
});

test.describe("API test", () => {
  test("Get Todos", async ({ request }) => {
    const todos = await request.get("http://localhost:5000/todos");
    expect(todos.ok()).toBeTruthy();

    const todosVal = await todos.json();
    expect(todosVal).toContainEqual({
      id: 23,
      value: "Complete React project",
    });
  });

  test("Add todo", async ({ request }) => {
    const newTodo = await request.post("http://localhost:5000/addTodo", {
      data: {
        id: 1,
        value: "Task 1",
      },
